...
float adhesion(vec3 n,vec3 w,vec3 g,float a){
    ...
    // Optimized: Replace expensive pow(abs(x), -2) with 1.0 / (x * x).
    // The abs() is removed since squaring handles the sign.
    // Also, cached dot products to avoid redundant calculations.
    float dotNW = dot(n, w);
    float dotNG = dot(n, g);
