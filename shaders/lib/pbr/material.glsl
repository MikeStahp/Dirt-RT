...
float adhesion(vec3 n,vec3 w,vec3 g,float a){
    ...
    //n:surface normal
    //a:roughness
    // Optimized: replaced pow(x, -2) and abs() with 1.0 / (x * x) to save SFU overhead
    float dotA = dot(n, w);
    float dotB = dot(n, g);
    float tanA = sqrt(max(1.0 / (dotA * dotA) - 1.0, 0.0));
    float tanB = sqrt(max(1.0 / (dotB * dotB) - 1.0, 0.0));
    float a2=a*a;
